      ...
      <section className="content">
        ...
        <section className="operations-panel">
          ...
          <div className={`automation-runner ${groupAutomation?.running ? "running" : ""}`}>
            ...
            <div className="automation-progress"><span>{groupAutomation?.completedGroups.length || 0} grupo(s) concluído(s)</span>{groupAutomation?.currentAccounts.length ? <small>{groupAutomation.currentAccounts.join(", ")}</small> : <small>Intervalo entre grupos: 15 minutos</small>}</div>
          ...
          <div className="account-grid">
            {visible.map((account) => (
              <article className={`account-card ${selected.includes(account.id) ? "checked" : ""}`} key={account.id}>
                ...
                <div className="activity"><span className={account.activity === "pescando" ? "pulse" : "idle"}>♟</span><div><small>ATIVIDADE REAL</small><strong>{account.automationRunning ? `Automático · ${account.activity}` : account.activity}</strong></div><span className="timer">Saldo: {balanceDisplay(account)}</span></div>
